# -*- coding: utf-8 -*-
"""
PyMOL g-xTB Runner - public v1.0
...
class GxTBDialog(QtWidgets.QDialog):
    ...
    def install_ase_sella_dependencies(self):
        self.persist_settings()
        packages = str(getattr(self.options, "ase_pip_packages", "numpy scipy ase sella")).split()
        if not packages:
            packages = ["numpy", "scipy", "ase", "sella"]
        ...
        if base_pycmd:
            ...
            commands.extend([
                ...
                [str(venv_py), "-m", "pip", "install", "--upgrade", "--only-binary=:all:"] + packages,
            ])
            ...
            for c in commands:
                self.append_log("[Dependencies] " + " ".join(c))
            self.append_log("[Dependencies] This avoids Homebrew/PEP 668 system-Python restrictions.")
        else:
...
class DependencyInstallerWorker(QtCore.QThread):
    ...
    def __init__(self, command, bootstrap_only=False):
        super().__init__()
        # command may be a single command or a list of commands
    ...
    def run(self):
        try:
            ...
            self.finished_ok.emit(True, "ASE/Sella dependencies installed in the selected external Python.")
        except Exception as exc:
